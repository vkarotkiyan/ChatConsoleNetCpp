...
uint* sha1(const char* message, uint msize_bytes) {
    uint A = H[0]; //инициализация
    uint B = H[1];
    uint C = H[2];
    uint D = H[3];
    uint E = H[4];

    uint totalBlockCount = msize_bytes / one_block_size_bytes; // подсчет целого числа блоков

    uint needAdditionalBytes = // подсчет, сколько байт нужно, чтобы дополнить последний блок
    ...
    *ptr_to_size = bring_to_human_view(msize_bytes * 8);
    ...
    for (int i = 0; i < totalBlockCount; i++) {
        ...
        uint d = H[3];
        uint e = H[4];
        for (int j = 0; j < block_expend_size_uints; j++) { // пересчитываем
            uint f;
            uint k;
            if (j < 20) { // в зависимости от раунда считаем по-разному
                f = (b & c) | ((~b) & d);
                k = 0x5A827999;
            }
            else if (j < 40) {
